...
export default function Footer() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setOpacity(Math.min(1, progress * 2))
    }
  ...
  return (
    <footer className="bg-black border-t border-white/5">
      {/* CTA Band with scroll-scale LUKES */}
      <div className="relative py-32 overflow-hidden" ref={ctaRef}>
        {/* Big LUKES that scales up on scroll */}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none">
          <span
            className="font-display font-light text-white/5 whitespace-nowrap"
            style={{
        ...
        <div className="relative z-10 text-center">
          ...
          >
            Join the <em className="italic">Inner Circle</em>
          </h2>
          ...
          <div>
            ...
            {!submitted ? (
              <form onSubmit={handleSubmit} className="flex gap-0 mb-6">
                <input type="email" placeholder="Email address" value={email} onChange={(e) => setEmail(e.target.value)} className="flex-1" required />
              ...
              <div className="border border-white/10 bg-white/5 p-4 mb-6">
                <p className="text-white text-sm">You're on the list. We'll be in touch.</p>
              </div>
            )}
            <div>
              ...
              <div className="flex gap-3">
                {['X', 'In', 'Be', 'Ig'].map((s) => (
                  <button key={s} className="w-10 h-10 rounded-full border border-white/10 text-white/40 text-xs hover:border-white hover:text-white transition-all duration-300 font-bold">{s}</button>
                ))}
              </div>
            </div>
          </div>
