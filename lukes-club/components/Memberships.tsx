    ...
    >
      {isHovered && (
        <div
          ...
          style={{ background: `radial-gradient(circle at ${glowPos.x}% ${glowPos.y}%, rgba(255,255,255,0.07) 0%, transparent 60%)` }}
        />
      )}
      {children}
    </div>
          ...
          {tiers.map(({ num, name, tagline, price, period, description, features, color, bg, cta, featured }, i) => (
            ...
            >
              <TiltCard featured={featured}>
                <div className={`tier-card relative flex flex-col ring-1 ${featured ? 'ring-white/30' : 'ring-white/10'}`}>
                  {featured && (
                  ...
                  <div className="px-8 pb-8">
                    <button
                      ...
                      style={{ borderColor: color, color: '#fff', background: 'transparent' }}
                      onMouseEnter={(e) => { e.currentTarget.style.background = 'white'; e.currentTarget.style.color = '#000'; e.currentTarget.style.borderColor = 'white' }}
                      onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#fff'; e.currentTarget.style.borderColor = color }}
                    >
                      {cta} →
                    </button>
                  </div>
